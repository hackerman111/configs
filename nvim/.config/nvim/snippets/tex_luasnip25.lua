...
-- Совместимость: LuaSnip v2.5+ и from_lua loader.
--
-- Содержимое сниппетов, triggers и вставляемый LaTeX сохранены.
-- Обвязка нормализует устаревающий regTrig, безопасно разделяет
-- обычные и автоматические сниппеты и использует актуальный доступ
-- к данным родительского snippet в functionNode/dynamicNode.

local ls = require("luasnip")
local raw_snippet = ls.snippet
...
-- LuaSnip рекомендует trigEngine = "pattern" вместо regTrig = true.
-- Определения ниже оставлены без изменений; преобразование выполняется здесь.
local snippet_types = setmetatable({}, { __mode = "k" })
...
local function s(context, nodes, opts)
    local normalized = copy_table(context)
    local snippet_type = "snippet"

    if type(normalized) == "table" then
        snippet_type = normalized.snippetType or snippet_type
        if normalized.regTrig == true and normalized.trigEngine == nil then
            normalized.trigEngine = "pattern"
            normalized.regTrig = nil
        end
    end

    local snippet = raw_snippet(normalized, nodes, opts)
    snippet_types[snippet] = snippet_type
    return snippet
end

local function enclosing_snippet(parent)
    return parent.snippet or parent
end
...
local function ms(trig_opts, nodes, opts)
    ...
    if type(trig_opts) == "string" then
        ...
        context = copy_table(trig_opts)
        context.snippetType = context.snippetType or "autosnippet"
    end

    local snippet_opts = copy_table(opts or {})
    local original_condition = snippet_opts.condition
    if original_condition then
        snippet_opts.condition = function(...)
            return in_mathzone() and original_condition(...)
        end
    else
        snippet_opts.condition = in_mathzone
    end

    return s(context, nodes, snippet_opts)
end
...
local function md_list_to_latex(_, snip)
    local lines = enclosing_snippet(snip).env.TM_SELECTED_TEXT
...
snippets.general_math = {
    ...
    s({
        ...
        -- Данные trigger хранятся в окружающем snippet.
        local num = enclosing_snippet(parent).captures[1]
    ...
    s({
        ...
        -- Полное совпадение trigger берётся из окружающего snippet.
        local stripped = enclosing_snippet(parent).trigger:sub(1, -2)
...
-- Сборка и возврат
-- ──────────────────────────────────────────────────────────────
-- from_lua принимает два возвращаемых массива:
--   1. обычные сниппеты;
--   2. autosnippets.
-- Явный порядок групп исключает зависимость от порядка pairs().
...
local function build_snippet_lists()
    local regular = {}
    local autosnippets = {}

    for _, category_name in ipairs(category_order) do
        local category = assert(
            snippets[category_name],
            ("Не найдена категория сниппетов: %s"):format(category_name)
        )

        for _, snippet in ipairs(category) do
            if snippet_types[snippet] == "autosnippet" then
                autosnippets[#autosnippets + 1] = snippet
            else
                regular[#regular + 1] = snippet
            end
        end
    end
